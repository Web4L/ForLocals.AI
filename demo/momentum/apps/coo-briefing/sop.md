...
## Prerequisites
- The day's data export exists in the `data/SCHEMA.md` shape.
  - **Mini-Phase 0.5:** export the tracking Google Sheet → JSON (one row set per
    day) and save as `data/briefing-YYYY-MM-DD.json`.
  - **Live:** the bridge/reconcile job writes the same shape from MindBody + GHL.
- `brand/voice-tone.md` is current.
...
3. **Generate.** Run the prompt in `tools/prompt-library.md` with the day's JSON.
...
5. **Publish.** Save the result to `briefings/latest.md` (overwrites — this is
   what the dashboard shows) **and** archive a dated copy
